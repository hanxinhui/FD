...
//            [btn setTitleColor:UIColorFromRGBValue(0x999999) forState:UIControlStateNormal];
            [btn setTitleColor:UIColorWithRGB(88, 88, 88, 1) forState:UIControlStateNormal];
            btn.titleLabel.font = [UIFont systemFontOfSize:15];
//            [btn setTitleColor:UIColorFromRGBValue(0x454545) forState:UIControlStateSelected];
            [btn setTitleColor:UIColorWithRGB(228, 89, 68, 1) forState:UIControlStateSelected];
            [btn setTitle:[titles objectAtIndex:i] forState:UIControlStateNormal];
            [btn addTarget:self action:@selector(segmentedControlChange:) forControlEvents:UIControlEventTouchUpInside];
            [btn setBackgroundImage:[UIImage imageNamed:@"select_up.png"] forState:UIControlStateNormal];
            [btn setBackgroundImage:[UIImage imageNamed:@"unSelect_down.png"] forState:UIControlStateSelected];
...
//            [_scrollView addSubview:lineView];
        ...
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(10.0f, HYSegmentedControl_Height-2, width4btn-25.0f, 1.0f)];
        _bottomLineView.backgroundColor = UIColorFromRGBValue(0xEE7621);
        

       [_scrollView addSubview:_bottomLineView];
        
        
        UIImageView *lineImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, HYSegmentedControl_Height-1, self.frame.size.width, 1)];
        lineImgView.backgroundColor = UIColorWithRGB(230, 230, 231, 1);
...
- (void)segmentedControlChange:(UIButton *)btn
{
    ...
    rect4boottomLine.origin.x = btn.frame.origin.x +10;
