...
@interface SnatchPayViewController ()


@end

@implementation SnatchPayViewController
...
- (void)viewDidLoad {
    ...
    _allLab = [[UILabel alloc] initWithFrame:CGRectMake(0, setHeight, iPhoneWidth - 62, 50)];
    ...
    [self setLabel:CGRectMake(iPhoneWidth - 60, setHeight, 60, 50) labFont:16 labText:@"葫芦币" textC:[UIColor redColor]];
    ...
    _vacanciesLab.text = @"(余额:0000000葫芦币)";
    ...
    _otherLab = [[UILabel alloc] initWithFrame:CGRectMake(0, setHeight, iPhoneWidth - 62, 50)];
    ...
    setHeight = setHeight + 50;
//    [self setBgImgView:CGRectMake(0, setHeight, iPhoneWidth, 50) imageName:nil];
    [self setBgImgView:CGRectMake(0, setHeight, iPhoneWidth, 70) imageName:nil];
...
- (void)gotoGetResult{
    ...
    payResultViewController.listDic = _resultDict;
...
- (void)reqCreateWerlfare{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setObject:[NSNumber numberWithInt:REQ_SNATCH_PAYNOW] forKey:REQ_CODE];
    ...
    if (isYuePay &&  yue > allPay) {
        ...
        [dict setObject:@"0" forKey:@"paytype"];
...
- (void)reqCreateResult{
    //
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setObject:[NSNumber numberWithInt:REQ_KWELFARE_RESULT_SNATCHPAY] forKey:REQ_CODE];
    [dict setObject:[NSString stringWithFormat:@"%ld",(long)[UserDataManager sharedUserDataManager].userData.UID] forKey:@"uid"];
    [dict setObject:[NSString stringWithFormat:@"%@",[_codeDict objectForKey:@"code"]] forKey:@"out_trade_no"];
    switch (_payStyle) {
        case publicPayStyle:
        {
            [dict setObject:@"0" forKey:@"kind"];
...
- (void)requestFinished:(NSDictionary *)resultDict
{
    ...
    switch ([[resultDict objectForKey:REQ_CODE] integerValue]) {
        ...
        case REQ_GETBALANCE_FREE:{
            ...
            // 显示余额
            _vacanciesLab.text = [NSString stringWithFormat:@"(余额:%.2f葫芦币)",[getY floatValue] ];
            yue = [getY floatValue] ;
            float sss;
            if ([[_payDict objectForKey:@"pice"] floatValue] < yue){
                sss = 0;
        ...
        case REQ_SNATCH_PAYNOW:
        {
            if (isYuePay &&  yue > allPay) {
                ...
                NSArray  *topicList = [_resultDict objectForKey:RESP_LIST];
                ...
                [_resultDict setObject:dataArr forKey:RESP_LIST];
                [self gotoGetResult];
...
//            [self reqGetBalance];
            ...
            // 第三方支付成功
            _resultDict = [NSMutableDictionary dictionaryWithDictionary:resultDict];
            [self gotoGetResult];
        }
            break;
        default:
