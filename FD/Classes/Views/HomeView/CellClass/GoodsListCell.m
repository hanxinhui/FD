...
- (void)creat{
    ...
    // 图片
    _headImgView = [[UIImageView alloc] initWithFrame:CGRectMake(10, 20  , 90, 90 )];
    _headImgView.backgroundColor = [UIColor clearColor];
    [self addSubview:_headImgView];
    
    
    // 标题
    _titleLab = [[UILabel alloc] initWithFrame:CGRectMake(110, 15, iPhoneWidth - 120, 30)];
    ...
    // 兑换价格
    [self setTheLab:CGRectMake(110, 75, 80, 20) textColor:UIColorWithRGB(155, 155, 155, 1) labText:@"兑换价格" setFont:14 setCen:NO];
    // 价格
    _priceLab= [[UILabel alloc] initWithFrame:CGRectMake(110, 95, iPhoneWidth - 220, 20)];
    ...
    // 库存
    [self setTheLab:CGRectMake(iPhoneWidth - 120, 75, 80, 20) textColor:UIColorWithRGB(155, 155, 155, 1) labText:@"商品库存" setFont:14 setCen:YES];

    // 任务奖励
    _stockLab= [[UILabel alloc] initWithFrame:CGRectMake(iPhoneWidth - 120, 95, 80, 20)];
...
//TODO:获取数据
- (void)setNode:(GoodsListNode *)node{
    if (_node == node) return;
    
    _node = node;
    
    self.titleLab.text = _node.Gname;
    self.desLab.text = _node.Gdesc;
    self.priceLab.text = _node.Gprice;
    self.stockLab.text = _node.Gstock;

    [self.headImgView sd_setImageWithURL:[NSURL URLWithString:node.Gthumb] placeholderImage:[UIImage imageNamed:@"Public_list_noImg.png"]];
}
