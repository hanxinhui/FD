...
@implementation GoodsBtnScrollView
...
- (void)initWithNameButtons
{
    ...
    for (int i = 0; i < [self.nameArray count]; i++) {
        ...
        CateNode *node = [self.nameArray objectAtIndex:i];
        NSString *title = node.Cname;
        NSInteger ttt = [node.Cid integerValue];
        [button setTag:ttt+ _varTag];
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    NSInteger atag = 0;
    for (int i = 0; i<_buttonOriginXArray.count; i++) {
        CateNode *node = [self.nameArray objectAtIndex:i];
        NSInteger ttt = [node.Cid integerValue];
        if (BUTTONID- _varTag == ttt) {
            atag = i;
        }
    }
    //按钮选中状态
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [shadowImageView setFrame:CGRectMake(sender.frame.origin.x + 20, 43, [[_buttonWithArray objectAtIndex:atag] floatValue] - 40, 1)];
