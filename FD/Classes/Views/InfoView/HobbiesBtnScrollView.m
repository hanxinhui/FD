...
@implementation HobbiesBtnScrollView
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (finished) {
                if (_btnDelegate && [_btnDelegate respondsToSelector:@selector(HobbiesSelectBtnTag:)]) {
                    UIButton *btn = (UIButton *)sender;
                    NSInteger tag = btn.tag - _varTag;
                    HobbiesChildNode *node = [self.nameArray objectAtIndex:tag];
                    
                    [_btnDelegate HobbiesSelectBtnTag:[node.Hid integerValue]];
                }
            }
        }];
